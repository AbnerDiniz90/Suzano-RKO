...
if __name__ == "__main__":
    ...
    def verificar_consistencia_carga(solucao_final):
        """
        Para cada promotor:
        - Recalcula o tempo total de cada dia (visitas + deslocamento)
        - Compara com o valor armazenado internamente
        - Reporta inconsistências
        """

        print("\n" + "="*60)
        print("      VERIFICAÇÃO DE CONSISTÊNCIA DE CARGAS      ")
        print("="*60)
        ...
        for i, promotor in enumerate(solucao_final):
            ...
            dias_coords = [
                ("Segunda", promotor.coords_segunda, promotor.carga_segunda),
                ("Terça",   promotor.coords_terca,   promotor.carga_terca),
                ("Quarta",  promotor.coords_quarta,  promotor.carga_quarta),
                ("Quinta",  promotor.coords_quinta,  promotor.carga_quinta),
                ("Sexta",   promotor.coords_sexta,   promotor.carga_sexta),
                ("Sábado",  promotor.coords_sabado,  promotor.carga_sabado)
            ]

            print(f"\nPROMOTOR {i}")

            for nome_dia, coords_dia, carga_armazenada in dias_coords:

                # Soma do tempo de visitas do dia
                # (usando as cargas armazenadas como referência)
                if nome_dia == "Segunda":
                    cargas = promotor.cargas_segunda
                elif nome_dia == "Terça":
                    cargas = promotor.cargas_terca
                elif nome_dia == "Quarta":
                    cargas = promotor.cargas_quarta
                elif nome_dia == "Quinta":
                    cargas = promotor.cargas_quinta
                elif nome_dia == "Sexta":
                    cargas = promotor.cargas_sexta
                else:
                    cargas = promotor.cargas_sabado

                soma_visitas = sum(cargas)

                # Recalcula custo de deslocamento daquele dia
                custo_esperado = soma_visitas + promotor.custo_viagem(coords_dia)

                # Comparar com o valor oficial armazenado
                diferenca = abs(custo_esperado - carga_armazenada)

                print(f"  Dia: {nome_dia}")
                print(f"    >> Calculado:        {custo_esperado:.2f} min")
                print(f"    >> Armazenado:       {carga_armazenada:.2f} min")
        ...
        if not inconsistencias:
            print("\n[SUCESSO] Todas as cargas conferem perfeitamente.\n")
        else:
            print("\n[FALHA] Foram encontradas inconsistências. Verifique acima.\n")
    #========================================
    ...
    verificar_consistencia_carga(solucao_final)