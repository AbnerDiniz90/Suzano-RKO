...
class Promotores:
    ...
    def distancia_por_dia(self, dia):
        """Retorna a distância percorrida em um dia específico."""
        if dia == 0:
            return self.distancia(self.coords_segunda)
        elif dia == 1:
            return self.distancia(self.coords_terca)
        elif dia == 2:
            return self.distancia(self.coords_quarta)
        elif dia == 3:
            return self.distancia(self.coords_quinta)
        elif dia == 4:
            return self.distancia(self.coords_sexta)
        elif dia == 5:
            return self.distancia(self.coords_sabado)
        else:
            return 0
        
    def lojas_por_dia(self, dia):
        """Retorna a lista de coordenadas visitadas no dia, na ordem da rota."""
        if dia == 0:
            return self.coords_segunda
        elif dia == 1:
            return self.coords_terca
        elif dia == 2:
            return self.coords_quarta
        elif dia == 3:
            return self.coords_quinta
        elif dia == 4:
            return self.coords_sexta
        elif dia == 5:
            return self.coords_sabado
        else:
            return []
...
if __name__ == "__main__":
    ...
    def tabela_distancias_por_dia(solucao_final, list_coords):
        dias_nome = ["Seg", "Ter", "Qua", "Qui", "Sex", "Sab"]

        print("\n" + "="*110)
        print("               DISTÂNCIA E ROTAS POR PROMOTOR E POR DIA")
        print("="*110)
        ...
        for i, promotor in enumerate(solucao_final):
            ...
            # Cabeçalho
            print("{:<8} {:>12}   {}".format("Dia", "Distância", "Lojas Visitadas (na ordem)"))
            print("-"*110)

            for d in range(6):
                dist = promotor.distancia_por_dia(d)
                lojas_coords = promotor.lojas_por_dia(d)

                # Converte coordenada -> ID da loja
                if lojas_coords:
                    lojas_ids = []
                    for coord in lojas_coords:
                        try:
                            id_loja = list_coords.index(coord)
                        except ValueError:
                            id_loja = "?"
                        lojas_ids.append(str(id_loja))

                    lojas_str = ", ".join(lojas_ids)
                else:
                    lojas_str = "-"

                print("{:<8} {:>12.2f}   {}".format(
                    dias_nome[d], dist, lojas_str
                ))

            print("-"*110)

    tabela_distancias_por_dia(solucao_final, list_coords)
